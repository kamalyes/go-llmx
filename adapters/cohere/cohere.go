 ...
 * @Date: 2026-07-02 22:31:29
 * @LastEditors: kamalyes 501893067@qq.com
 * @LastEditTime: 2026-07-02 22:47:11
 * @FilePath: \go-llmx\adapters\cohere\cohere.go
 * @Description: Cohere 适配器 —— v2 chat 协议实现（含 SSE 流式）.
 * tool 消息双重结构（tool_calls 引用 + tool_result 内容）收口在 wire 编解码
 *
...
// New 构造客户端（默认 command-r-plus-08-2024）.
// [EN] Build a client.
...
func (c *Client) GenerateContent(ctx context.Context, messages []llmx.Message, opts ...llmx.Option) (*llmx.Response, error) {
	...
	if err := c.TC.PostJSON(ctx, c.GetEndpoint(), req, &wr, c.headers()); err != nil {
		return nil, adapter.MapTransportError(err, cohereClassifier{})
	}
	if len(wr.Message.Content) == 0 && len(wr.Message.ToolCalls) == 0 {
...
func (c *Client) StreamGenerateContent(ctx context.Context, messages []llmx.Message, stream llmx.StreamHandler, opts ...llmx.Option) (*llmx.Response, error) {
	...
	if err != nil {
		return st.response(model), adapter.MapTransportError(err, cohereClassifier{})
	}
