 ...
 * @Date: 2026-05-25 20:39:33
 * @LastEditors: kamalyes 501893067@qq.com
 * @LastEditTime: 2026-05-25 20:39:33
 * @FilePath: \go-llmx\adapters\anthropic\logging_test.go
 * @Description: ctx + logger 端到端测试 —— 非流式/流式打点双维度、业务 ctx 追踪值贯通、
 * 取消语义跨层保留. mock 基建见 anthropic_test.go
 *
...
func TestE2E_LoggingAndContext(t *testing.T) {
	ctx := context.WithValue(context.Background(), ctxKey{}, "trace-anthropic")
	...
	cl := newE2ELogger()
	c := New("k", WithBaseURL(m.srv.URL), WithModel("claude-sonnet-4-5"), WithLogger(cl))
	...
	assertTraceFlows(t, entries, "trace-anthropic")

	// 流式：[LLMX] stream + http 请求完成
